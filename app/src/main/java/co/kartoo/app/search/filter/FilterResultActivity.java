...
@EActivity(R.layout.activity_filter_result)
public class FilterResultActivity extends AppCompatActivity
    ...
    ArrayList<SearchPromotion> listTrendingPromo;
    PromoFromFilterAdapter adapterTrendingPromo;
...
//        sequence.start();
        ...
        isMyCards = intent.getStringExtra("isMyCard");
        //Log.e("TAG", isMyCards);
        sort = intent.getStringExtra("sortBy");
        categoryList = intent.getStringArrayListExtra("categoryTypes");
...
//        }
        ...
        adapterTrendingPromo = new PromoFromFilterAdapter(this, listTrendingPromo, promoService, token, mRVpromo);
...
//                if (pos==0){
//                    if (myLocation.longitude==0){
//                        Toast.makeText(FilterResultActivity.this, "Are you lost? Please turn on your location", Toast.LENGTH_LONG).show();
//                        sortbySpinner.setSelection(a);
//                    }
//                    else {
//                        sortValue = "distance";
...
//        mTBmyCards.setOnCheckedChangeListener(FilterResultActivity.this);
    ...
    public void checkPromos(){
        ...
        FilterSearch passParameter = new FilterSearch();
        passParameter.setIsMyCard(isMyCards);
        passParameter.setSortBy(sort);
        passParameter.setCategoryTypes(categoryList);

        Call<SearchPromotionHeader> favoriteOutletCall = promoService.doFilterSearch(token, 1, passParameter);
        favoriteOutletCall.enqueue(new Callback<SearchPromotionHeader>() {
            @Override
            public void onResponse(Response<SearchPromotionHeader> response, Retrofit retrofit) {
                if (response.isSuccess()) {
                    if (response.code() == 200) {
                        ...
                        Toast.makeText(FilterResultActivity.this, "Your session has expired, please login again", Toast.LENGTH_LONG).show();
                        ...
                        Intent intent2 = new Intent(FilterResultActivity.this, LandingActivity_.class);
...
//        }
        ...
        Call<SearchPromotionHeader> promoByCategoryCall;
        //if (myCards){
        if (sortValue.equals("distance"))
        {
            promoByCategoryCall = promoService.doFilterSearch(token, 1, passParameter);
        }
        else
        {
            promoByCategoryCall = promoService.doFilterSearch(token, 1, passParameter);
        ...
        promoByCategoryCall.enqueue(new Callback<SearchPromotionHeader>() {
            @Override
            public void onResponse(Response<SearchPromotionHeader> response, Retrofit retrofit) {
                if (response.isSuccess()) {
            ...
            public void onScrollChanged() {
                ...
                if (diff < 0) {
                    if (!loading && lastPage < maxPage) {
                        ...
                        if (sortValue.equals("distance"))
                        {
                            promoByCategoryCall = promoService.doFilterSearch(token, lastPage+1, passParameter);
                        }
                        else
                        {
                            promoByCategoryCall = promoService.doFilterSearch(token, lastPage+1, passParameter);
                        ...
                        //   Call<CategoryOutlet> promoByCategoryCall = promoService.u1(token, id, lastPage+1, sortValue, String.valueOf(myLocation.longitude), String.valueOf(myLocation.latitude));
                        promoByCategoryCall.enqueue(new Callback<SearchPromotionHeader>() {
                            @Override
                            public void onResponse(Response<SearchPromotionHeader> response, Retrofit retrofit) {
                                loading = false;
                                if (response.isSuccess()) {
                                    if (response.code() == 200) {
                                        ...
                                        maxPage = Integer.valueOf(response.body().getMaxPage());
                                        lastPage++;
                                    }
                                }
                            }
...
//                        }
    ...
    public void onEvent(TrendingFinishedEventFilterResult event) {
